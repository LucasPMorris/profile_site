...
const BlogHeader = ({ title, page_views_count, published_at, reading_time_minutes }: BlogHeaderProps) => {
  ...
  const transition = { duration: 0.3, ease: easeInOut }; // Predefined easing function
  const titleVariants = { initial: { opacity: 0, y: -20 }, animate: { opacity: 1, y: 0 } };

  return (
    <>
      {!isScrolled ? (
        <motion.h1 className='text-2xl font-semibold' initial='initial' animate='animate' variants={titleVariants} transition={transition}>{title}</motion.h1>
      ) : (
        <motion.div className='shadow-bottom top-0 z-10 border-b border-neutral-300 bg-light py-6 backdrop-blur dark:border-neutral-600 dark:bg-dark lg:sticky' initial='initial' animate='animate' variants={titleVariants} transition={transition}>
