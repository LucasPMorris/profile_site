...
const BlogFeaturedSection = () => {
  const { data, isLoading } = useSWR( `/api/blog?page=1&per_page=4&categories=11`, fetcher, { revalidateOnFocus: false, refreshInterval: 0 } );

  const featuredData: BlogItemProps[] = useMemo(() => {
    if (data?.status && data?.data?.posts && Array.isArray(data?.data?.posts)) { return data?.data?.posts; }
    return [];
  }, [data]);
