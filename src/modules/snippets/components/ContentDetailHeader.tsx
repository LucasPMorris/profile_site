...
const ContentDetailHeader = ({ title, category, source, difficulty, language, source_url }: SubContentMetaProps) => {
  ...
  return (
    <>
      ...
      <div className='mb-6 flex flex-col items-start justify-between gap-2 border-b border-dashed ml-2 border-neutral-600 pb-6 pt-3 text-[14px] text-neutral-600 dark:text-neutral-400 sm:flex-row lg:items-center'>
        <div># {category}</div>
        <div className='mt-1 flex items-center gap-4'>
          {source && source_url && (
            <Link href={source_url} target='_blank' passHref>
              <div className='flex items-center gap-2 font-medium text-neutral-800 dark:text-neutral-300 '>
                <LinkIcon size={18} />
                <span className='text-[15px] transition-all duration-300 dark:text-teal-500 hover:dark:text-teal-400'>View in {source}</span>
              </div>
            </Link>
          )}
          {difficulty && (
