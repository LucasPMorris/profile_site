...
const CareerCard = ({ position, company, company_legal_name, logo, location, location_type, start_date, end_date, link, type, responsibilities }: CareerProps) => {
  ...
  return (
    <Card className='flex gap-5 border border-neutral-400 px-6 py-4 dark:border-neutral-900'>
      <div className='mt-1.5 w-fit'>
        {logo ? (<Image src={logo} width={55} height={55} alt={company} objectFit="contain" className='h-14 w-14 rounded bg-neutral-50 p-1 hover:scale-110 hover:bg-transparent'/>)
        : (<CompanyIcon size={50} />)}
      </div>
          ...
          <div className='space-y-1 text-sm text-neutral-600 dark:text-neutral-400'>
            <div className='flex flex-col gap-1 md:flex-row md:items-center md:gap-2'>
              <a href={link || '#'} target='_blank' data-umami-event={`Click Career Company Name: ${company}`}>
                <span className='cursor-pointer underline-offset-2 hover:text-dark hover:underline hover:dark:text-white'>{company}</span>
              </a>
              <span className='hidden text-neutral-300 dark:text-neutral-700 lg:block'> • </span>
              <span className='text-neutral-500'>[ {location_type} ]</span>
              <span className='hidden text-neutral-300 dark:text-neutral-700 lg:block'> • </span>
              <span>{location}</span>
            </div>
            <div className='flex flex-col gap-2 md:flex-row md:text-[13px]'>
              <div className='flex gap-1'><span>{startDateFormatted} - {endDateFormatted}</span></div>
              <span className='hidden text-neutral-300 dark:text-neutral-700 lg:block'> • </span>
