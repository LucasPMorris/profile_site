...
const Skills = () => { const [shuffledSkills, setShuffledSkills] = useState< Array <[string, ReactNode]> >([]);
  ...
  const sliders = Array.from({ length: 1 }, (_, index) => {
    ...
    return (
      <InfiniteLoopSlider key={index} isReverse={index === 1}>
        {sliderSkills.map(([title, icon], index) => ( <Tag key={index} icon={icon} title={title} /> ))}
      </InfiniteLoopSlider>
    );
  });
    ...
    <div className='space-y-8'>
      <div className='flex w-full'>
        <div className='relative flex w-full flex-col justify-start gap-y-4 overflow-hidden py-2'>
          <SliderWrapper className='flex w-full'>
            {sliders}
          </SliderWrapper>
        </div>
      </div>
