...
interface DevicePopoverProps { devices: DeviceProps[]; isShow: boolean; }

const DevicePopover = ({ devices, isShow }: DevicePopoverProps) => {
  const listDevices = devices?.map((device) => ({ ...device,
    icon: PAIR_DEVICES[device?.type]?.icon || (<DeviceIcon className='w-auto text-neutral-800 dark:text-neutral-300' size={24} /> ),
    model: PAIR_DEVICES[device?.type]?.model || 'Unknown Device',
  }));

  return (
    <Transition as={Fragment} show={isShow} enter='transition ease-out duration-200' enterFrom='opacity-0 translate-y-1' enterTo='opacity-100 translate-y-0' leave='transition ease-in duration-150' leaveFrom='opacity-100 translate-y-0' leaveTo='opacity-0 translate-y-1' >
