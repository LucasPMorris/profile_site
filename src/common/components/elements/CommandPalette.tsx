...
const CommandPalette = () => {
  ...
  const menuOptions: MenuOptionProps[] = [
    ...
    {
      title: 'SOCIALS',
      children: SOCIAL_MEDIA?.map((menu) => ({...menu, closeOnSelect: true,
      })),
    },
    {
      title: 'APPEARANCE',
  ...
  const handleSelect = (menu: MenuOptionItemProps) => {
    ...
    if (menu.isExternal) { window.open(menu.href, '_blank');
    } else { router.push(menu?.href as string); }
  };
  ...
  useEffect(() => {
    if (!isOpen) { setQuery(''); setEmptyState(false); handleAiClose(); }
  }, [isOpen]);

  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'k' && (event.metaKey || event.ctrlKey)) { setIsOpen(!isOpen); }
            ...
            >
              ...
              {askAssistantClicked &&
                ...
                filterMenuOptions.every(
                  ...
                  <div className='max-h-80 overflow-y-auto px-8 py-7 text-neutral-800 dark:text-neutral-300'>
                    {aiLoading ? 
                      ( <AiLoading /> )
                      : (<AiResponses response={aiResponse} isAiFinished={aiFinished} onAiFinished={() => setAiFinished(true)} onAiClose={handleAiClose} /> )}
                  </div>
                )}
            </Combobox>
          </Transition.Child>
