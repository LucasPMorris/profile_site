...
const NowPlayingCard = ({ isExpand = false }: { isExpand?: boolean }) => {
  ...
  return (
    <div
      className={clsx('z-2 fixed bottom-0 w-full p-3', !expand && 'flex justify-end')}>
      {!expand ? (
        <div className='m-2 cursor-pointer rounded-full bg-neutral-950 transition-all duration-100' onClick={handleMusicToggle}>
        ...
        <div className='mt-5 flex items-center justify-between rounded-md bg-green-400 px-3 py-2  text-neutral-800 dark:bg-green-500 dark:text-neutral-900 '>
          <div className='flex items-center gap-3'>
            { data?.albumImageUrl && (
              <Image className='rounded-md' unoptimized alt={data?.album} src={data?.albumImageUrl} width={60} height={60} />
            ) }
            <div className='flex flex-col pt-0.5 hover:cursor-pointer hover:underline' onClick={() => handleOpenSongUrl(data?.songUrl)} >
          ...
          <div className='flex gap-3 pr-0.5'>
            <CloseIcon size={28} className='cursor-pointer pt-0.5 text-neutral-900' onClick={handleMusicToggle} />
          </div>
        </div>
      )}
    </div>
