  ...
  return (
    <StyledToggle className='flex'>
      <input checked={resolvedTheme === 'dark'}
        type='checkbox' className='mode-toggle' onChange={toggleTheme} id='switch-theme'
        data-umami-event={`Switch to ${ resolvedTheme === 'light' ? 'Dark' : 'Light' } Mode`}
      />
      <label className={clsx('mode-toggle-label', resolvedTheme === 'light' ? 'ring-1 ring-indigo-300' : '')} htmlFor='switch-theme'>
        <svg width='45' height='25' viewBox='0 0 300 170' xmlns='http://www.w3.org/2000/svg'>
          <defs>
            <linearGradient id='bg-night'><stop className='bg-stop-start' offset='0%' /><stop className='bg-stop-end' offset='100%' /></linearGradient>
            <filter id='glow'><feDropShadow dx='0' dy='0' stdDeviation='8' floodColor='#ffffff' floodOpacity='0.75' /></filter>
            <filter id='glow-mini'><feDropShadow dx='0' dy='0' stdDeviation='0.5' floodColor='#ffffff' floodOpacity='0.5' /></filter>
          </defs>
