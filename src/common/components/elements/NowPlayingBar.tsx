      ...
      <div className='flex justify-between bg-green-400 px-4 pb-0.5 pt-[2.5px] text-[14px] text-neutral-800 dark:bg-green-500 dark:text-neutral-900'>
        {playingData?.songUrl ? (
          <Popover className='relative'>
            <Popover.Button as='div' onMouseEnter={() => setShowPlayingInfo(true)} onMouseLeave={() => setShowPlayingInfo(false)} >
              <div className='flex items-center gap-2'>
                <AnimatedBars />
                <div className='hidden sm:block '>Now Playing :</div>
                <div className='flex items-center gap-2 transition-all duration-300'>
                  {playingData?.albumImageUrl && (
                    <Image className='rounded-sm' unoptimized alt={playingData?.album} src={playingData?.albumImageUrl} width={18} height={18} />
                  )}
                  <div className='flex gap-1 hover:cursor-pointer hover:underline' onClick={() => handleOpenSongUrl(playingData?.songUrl)} > 
        ...
        {playingData?.songUrl && (
          <Popover className='relative'>
            <Popover.Button as='div' onMouseEnter={() => setShowDeviceList(true)} onMouseLeave={() => setShowDeviceList(false)} >
              <div className='flex items-center gap-1'>
