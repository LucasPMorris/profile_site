...
export const formatDate = (date: string, type = 'MMMM dd, yyyy') => {
  ...
  const formattedDate = format( utcToZonedTime(parseISO(date), 'Asia/Jakarta'), type ); return formattedDate };
...
export const groupContentByChapter = ( contents: MdxFileContentProps[] ): Record<string, ChapterGroupProps> => {
  ...
  return contents.reduce( (acc, content) => {
      ...
      if (!acc[chapter_id]) { acc[chapter_id] = { chapter_id, chapter_title, contents: [] }; }
...
export const parseUrl = (url: string): ParsedUrlProps => {
  ...
  return { parentSlug: parts[2], contentSlug: parts[3] };
};
