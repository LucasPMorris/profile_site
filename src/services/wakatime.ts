...
export const getReadStats = async (): Promise<{ status: number; data: any; }> => {
  const response = await axios.get(`${STATS_ENDPOINT}/last_7_days`, { headers: { Authorization: `Basic ${API_KEY}` } });
  ...
  const best_day = { date: getData?.data?.best_day?.date, text: getData?.data?.best_day?.text };
  ...
  return { status, data: { last_update, start_date, end_date, categories, best_day, human_readable_daily_average, human_readable_total, languages, editors } };
};

export const getALLTimeSinceToday = async (): Promise<{
  status: number; data: any; }> => { const response = await axios.get(ALL_TIME_SINCE_TODAY, { headers: { Authorization: `Basic ${API_KEY}` } });
  const status = response.status;

  if (status >= 400) return { status, data: {} };
  const getData = response.data;
  const data = { text: getData?.data?.text, total_seconds: getData?.data?.total_seconds };
  return { status, data };
};
