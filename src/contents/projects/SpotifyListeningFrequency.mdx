...
export async function validateBuckets() {
  ...
  if (issues === 0) {
    log('✅ All bucket hierarchy links are valid.', logPath);
  } else {
    log(`⚠️ Validation complete: ${issues} issues found.`, logPath);
  }
}
```
...
export async function updateBucketedStats(dryRun: boolean = false) {
  ...
  for (const month of monthBuckets) {
    ...
    if (!dryRun && artistRows.length > 0) {
      ...
      log(`[Prisma] Batch inserted trackstat for monthbucket=${month.id}, rows=${artistRows.length}`, logPath);
    }
